...
export default function LearningExperiencesListPage() {
      ...
      <section>
        ...
        <div className="lexlp-cards">
          {detailPages.map((page) => {
            ...
            return (
              <Link
                className="lexlp-card-link"
                key={page.id}
                to={`/learning-experience/${page.meta.slug}`}
              >
                <article className="lexlp-card">
                  <img
                    src={`${ENV.BASE_BACK_URL}${page.header_image.thumbnail.src}`}
                    alt={page.header_image.thumbnail.alt}
                  />
                  <div className="lexlp-card__details">
                    <p>{dateString}</p>
